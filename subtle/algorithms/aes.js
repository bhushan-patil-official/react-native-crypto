'use strict';
import { randomBytes as randomBytesCallback } from 'react-native-randombytes'
...
module.exports.AES_CTR = {
  ...
  encrypt(params, key, data) {
    ...
    return this._doCipher(iv, length, data, (data, iv) => {
      return aes.createCipheriv(cipher, secretKey, iv).update(data);
    });
  },
  ...
  decrypt(params, key, data) {
    ...
    return this._doCipher(iv, length, data, (data, iv) => {
      return aes.createDecipheriv(cipher, secretKey, iv).update(data);
    });
  }
};
...
module.exports.AES_CBC = {
  ...
  encrypt(params, key, data) {
    ...
    const c = aes.createCipheriv(cipher, secretKey, iv);
  ...
  decrypt(params, key, data) {
    ...
    const c = aes.createDecipheriv(cipher, secretKey, iv);
    try {
      return Buffer.concat([c.update(data), c.final()]);
    } catch (err) {
...
module.exports.AES_GCM = {
  ...
  encrypt(params, key, data) {
    ...
    try {
      const c = aes.createCipheriv(cipher, secretKey, ivBuf, { authTagLength });
  ...
  decrypt(params, key, data) {
    ...
    const authTagLength = tagLength === undefined ? 16 : tagLength >> 3;
    try {
      const c = aes.createDecipheriv(cipher, secretKey, ivBuf, { authTagLength });
      if (additionalData !== undefined)
        c.setAAD(additionalData);
